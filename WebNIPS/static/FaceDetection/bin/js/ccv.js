...
var ccv = {
	...
	detect_objects : parallable("ccv.js", function (canvas, cascade, interval, min_neighbors) {
		...
		function core(pyr, id, worker_num){
			...
			var scale_upto = this.shared.scale_upto;
			var i, j, k, x, y, q;
			var scale_fac = 1 * Math.pow(scale, start_ind)
			var dx = [0, 1, 0, 1];
			var dy = [0, 0, 1, 1];
			var seq = [];

			for (i = start_ind; i < scale_upto; i++) {
					...
					for (k = 0; k < cascade.stage_classifier[j].count; k++) {
						feature[k] = {
							"size" : orig_feature[k].size,
						  	...
						  	"pz" : orig_feature[k].pz,
						  	"nx" : new Array(orig_feature[k].size),
						  	"nz" : orig_feature[k].nz
						};
				...
				for (q = 0; q < 4; q++) {
					...
					for (y = 0; y < qh; y++) {
						for (x = 0; x < qw; x++) {
							...
							if (flag) {
								seq.push({"x" : (x * 4 + dx[q] * 2) * scale_fac,
										  "y" : (y * 4 + dy[q] * 2) * scale_fac,
										  "width" : cascade.width * scale_fac,
										  "height" : cascade.height * scale_fac,
										  "neighbor" : 1,
										  "confidence" : sum});
							}
							u8o[0] += 16;
		...
		function post(seq) {
			...
			for(i = 0; i < seq.length; i++) {
				var r1 = seq[i];
				var comp = comps[idx_seq[i]];

				if (comp.neighbors == 0) comp.confidence = r1.confidence;
				comp.neighbors++;

				comp.x += r1.x;
				comp.y += r1.y;
				comp.width += r1.width;
				comp.height += r1.height;
				if(r1.confidence > comp.confidence) comp.confidence = r1.confidence;
			}
